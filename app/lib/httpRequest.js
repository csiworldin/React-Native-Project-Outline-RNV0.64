...
const MakeApiRequest = (url, endpoint = null, data, headers = {}, method = "POST", isRaw = false, isJson = true, altURL = null, showError = true, responseTypeFile = false) => {
    ...
    return NetInfo.fetch().then(state => {
        if (state.isConnected) {
            ...
            if (showError) {
                Alert.error(Enums.GlobalMessages.networkRequestError);
            }
        }
    });
}
